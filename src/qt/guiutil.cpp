...
#define URI_SCHEME "socialtraders"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a socialtraders address (e.g. %1)").arg("BL4YpGWPRWYW5LC8QhC3BbjToaTVTP8Waa"));
#endif
...
{
    // return if URI is not valid or is no socialtraders: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::STRD, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert socialtraders:// to socialtraders:
    //
    //    Cannot handle this later, because socialtraders:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::STRD, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open socialtraders.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "socialtraders.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for socialtraders.lnk
...
{
    return GetAutostartDir() / "socialtraders.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a socialtraders.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=socialtraders\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the socialtraders app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add socialtraders app to startup item list
